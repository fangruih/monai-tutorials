    ...
    for epoch in range(n_epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            with autocast(enabled=True):
                ...
                if step % 2 == 0:
                    # print("in train file timesteps noise_x", timesteps)
                ...
                if step % 2 == 0:
                    with torch.no_grad():
                        # print("in train file timesteps noise_y", timesteps)
                        predicted_noise_y = unet(x=y_t, timesteps=timesteps, context=condition_x)
                else:
                    # print("in train file timesteps noise_y", timesteps)
                    predicted_noise_y = unet(x=y_t, timesteps=timesteps, context=condition_x)

                # Steps 10: calculate loss
                loss = F.mse_loss(predicted_noise_y.float(), noise_y.float()) + F.mse_loss(predicted_noise_x.float(), noise_x.float())

                # Step 11: predict age
                predicted_age = age_regressor(y_0_fake)
                loss += F.mse_loss(predicted_age.float(), age_y.float())
            ...
            with torch.no_grad():
                with autocast(enabled=True):
                    ...
                    for step, batch in enumerate(val_loader):
                        ...
                        # Steps 10: calculate loss
                        val_loss = F.mse_loss(predicted_noise_y.float(), noise_y.float()) + F.mse_loss(predicted_noise_x.float(), noise_x.float())
                        val_recon_epoch_loss += val_loss
                    ...
                    if rank == 0:
                        ...
                        if (epoch) % (2 * val_interval) == 0:  # time cost of synthesizing images is large
                            if condition_x!=None:
                                ...
                                # print("syntesize base on condition ", condition_x)
                                # print("condition shape ", condition_x.shape)
                            # print("noise_x", noise_x.shape)
                            # print("noise_x[0:1, ...]", noise_x[0:1, ...].shape)
                            # print("condition shape ", condition_x.shape)
